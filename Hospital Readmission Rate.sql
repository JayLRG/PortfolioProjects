...
FROM Discharges
WHERE Admission_Type NOT LIKE 'Elective'


--3: Readmissions by medical condition
--Need to account for a patient being non-electively admitted, discharged, then electively readmitted (Exclude elective readmissions despite being within 30 days)
SELECT medical_condition,
	   COUNT(medical_condition) AS 'Number of Readmissions'
FROM Discharges
WHERE Admission_Class LIKE 'Readmitted' AND Admission_Type NOT LIKE 'Elective'
GROUP BY Medical_Condition
ORDER BY COUNT(medical_condition) DESC, medical_condition


--4: Running readmission rate
--Need to again account for a patient being electively admitted
...
FROM Discharges
WHERE Admission_Type NOT LIKE 'Elective'
GROUP BY Date_of_Admission, Discharge_Date
ORDER BY Date_of_Admission, Discharge_Date


--Creating tables to produce queries 2, 3, and 4;
